...
const DashboardContainer = (props) => {
  ...
  // Mock useEffect;
  useEffect(() => {
    const retainedValues = mockData.retained_bytes.metrics.map(
      (topic) => topic.value
    );
    setTotal(mockData.retained_bytes.totalValue);
    setTopics({
      labels: mockData.retained_bytes.metrics.map((topic) => topic.topic),
      datasets: [
        {
          label: 'bytes',
          data: retainedValues,
    ...
    const valuesReq = mockData.request_bytes.metrics.map(
      (topic) => topic.value
    );
    const valuesRes = mockData.response_bytes.metrics.map(
      (topic) => topic.value
    );
    setTotalReq(mockData.request_bytes.totalValue);
    setTotalRes(mockData.response_bytes.totalValue);
    setReqRes({
      labels: mockData.request_bytes.metrics.map((topic) => topic.type),
      datasets: [
        {
          label: 'request bytes',
          data: valuesReq,
    ...
    dashboardView = (
      <main className='cluster-container'>
        {metricSelection.retainedBytes && (
          <>
            <Chart chartData={topicChart} totalBytes={totalBytes} />
            <TopicButtons
